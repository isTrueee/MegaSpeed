      ...
      <div class="flex flex-col w-full gap-4 sm:flex-row sm:items-center sm:gap-3 sm:w-auto">
        <div class="flex gap-3">
          <div>
            ...
            <div class="fixed inset-0 bg-[black]/60 z-[999] overflow-y-auto hidden" :class="addUserModal && '!block'">
              <div class="flex items-center justify-center min-h-screen px-4" @click.self="addUserModal = false">
                <div x-show="addUserModal" x-transition x-transition.duration.300 class="panel border-0 p-0 rounded-lg overflow-hidden md:w-full max-w-lg w-[90%] my-8">
                  <button type="button" class="absolute top-4 ltr:right-4 rtl:left-4 text-white-dark hover:text-dark" @click="addUserModal = false">
                  ...
                  <h3 class="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]" x-text="params.id ? 'Edit User' : 'Add User'"></h3>
                  <div class="p-5">
                    <form @submit.prevent="saveUser">
                      ...
                      <div class="flex items-center justify-end mt-8">
                        <button type="button" class="btn btn-outline-danger" @click="addUserModal = false">Cancel</button>
        ...
        <div class="relative ">
          <input type="text" placeholder="Search Users" class="py-2 form-input ltr:pr-11 rtl:pl-11 peer" x-model="searchUser" @keyup="searchUsers" />
                ...
                <tr>
                  <td>
                    <div class="flex items-center w-max">
                      <div x-show="user.path" class="w-max">
                        <img :src="`/assets/images/${user.path}`" class="object-cover w-8 h-8 rounded-full ltr:mr-2 rtl:ml-2" alt="avatar" />
                      </div>
                      <div x-show="!user.path && user.name" class="grid w-8 h-8 text-sm font-semibold text-white rounded-full place-content-center ltr:mr-2 rtl:ml-2 bg-primary" x-text="user.name.charAt(0) + '' + user.name.charAt(user.name.indexOf(' ') + 1)">
                      </div>
                      <div x-show="!user.path && !user.name" class="p-2 border border-gray-300 rounded-full dark:border-gray-800 ltr:mr-2 rtl:ml-2">
                  ...
                  <td>
                    <div class="flex items-center justify-center gap-4">
                      <button type="button" class="btn btn-sm btn-outline-primary" @click="editUser(user)">Edit</button>
          ...
          <div class="bg-white dark:bg-[#1c232f] rounded-md overflow-hidden text-center shadow relative">
            <div class="bg-white/40 rounded-t-md bg-[url('/assets/images/notification-bg.png')] bg-center bg-cover p-6 pb-0">
              <template x-if="user.path">
                <img class="object-contain w-4/5 mx-auto max-h-40" :src="`/assets/images/${user.path}`" />
              </template>
            </div>
            ...
            <div class="absolute bottom-0 flex w-full gap-4 p-6 mt-6 ltr:left-0 rtl:right-0">
              <button type="button" class="w-1/2 btn btn-outline-primary" @click="editUser(user)">Edit</button>
              <button type="button" class="w-1/2 btn btn-outline-danger" @click="deleteUser(user)">Delete</button>
            </div>
          </div>
        </template>
      </div>
  ...
  <script>
   document.addEventListener("alpine:init", () => {
			Alpine.data("users", () => ({
				...
				searchUsers() {
					this.filteredUserList = this.userList.filter((d) => d.name.toLowerCase().includes(this.searchUser.toLowerCase()));
				},
				editUser(user) {
				...
				saveUser() {
					...
					if (this.params.id) {
						...
						//add user
						let maxUserId = this.userList.length ? this.userList.reduce((max, character) => (character.id > max ? character.id : max), this.userList[0].id) : 0;
						let user = {
							id: maxUserId + 1,
							path: 'profile-35.png',
