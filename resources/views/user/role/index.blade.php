    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
      ...
      <div class="flex flex-col w-full gap-4 sm:flex-row sm:items-center sm:gap-3 sm:w-auto">
        <div class="flex gap-3">
          <div>
            <button type="button" class="btn btn-primary" @click="editUserRole">
							<svg class="w-5 h-5 ltr:mr-2 rtl:ml-2" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
								...
								<path stroke="#fff" stroke-linecap="round" stroke-width="1.5" d="M15 9a3 3 0 1 0 0-6M5.89 20.584C6.825 20.85 7.882 21 9 21c3.866 0 7-1.79 7-4s-3.134-4-7-4-7 1.79-7 4c0 .345.077.68.22 1M18 14c1.754.385 3 1.359 3 2.5 0 1.03-1.014 1.923-2.5 2.37"/>
								<path stroke="#fff" stroke-linecap="round" stroke-width="1.5" d="M3.55 18.4H4.95m0 0h1.3M4.95 18.4v-1.3M4.95 18.4v1.3"/>
							</svg>	
              Add User Role
            </button>
            <div class="fixed inset-0 bg-[black]/60 z-[999] overflow-y-auto hidden" :class="addUserRoleModal && '!block'">
              <div class="flex items-center justify-center min-h-screen px-4" @click.self="addUserRoleModal = false">
                <div x-show="addUserRoleModal" x-transition x-transition.duration.300 class="panel border-0 p-0 rounded-lg overflow-hidden md:w-full max-w-lg w-[90%] my-8">
                  <button type="button" class="absolute top-4 ltr:right-4 rtl:left-4 text-white-dark hover:text-dark" @click="addUserRoleModal = false">
                  ...
                  <h3 class="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]" x-text="params.id ? 'Edit User Role' : 'Add User Role'"></h3>
                  <div class="p-5">
                    <form @submit.prevent="saveUserRole">
                      ...
                      <div class="flex items-center justify-end mt-8">
                        <button type="button" class="btn btn-outline-danger" @click="addUserRoleModal = false">Cancel</button>
        ...
        <div class="relative ">
          <input type="text" placeholder="Search User Roles" class="py-2 form-input ltr:pr-11 rtl:pl-11 peer" x-model="searchUserRole" @keyup="searchUserRoles" />
      ...
      <template x-if="displayType === 'list'">
        <div class="table-responsive">
          <table class="table-striped table-hover">
            ...
            <tbody>
              <template x-for="userrole in filteredUserRoleList" :key="userrole.id">
                <tr>
                  <td>
                    <div class="flex items-center w-max">
                      <div x-show="userrole.path" class="w-max">
                        <img :src="`/assets/images/${userrole.path}`" class="object-cover w-8 h-8 rounded-full ltr:mr-2 rtl:ml-2" alt="avatar" />
                      </div>
                      <div x-show="!userrole.path && userrole.name" class="grid w-8 h-8 text-sm font-semibold text-white rounded-full place-content-center ltr:mr-2 rtl:ml-2 bg-primary" x-text="userrole.name.charAt(0) + '' + userrole.name.charAt(userrole.name.indexOf(' ') + 1)">
                      </div>
                      <div x-show="!userrole.path && !userrole.name" class="p-2 border border-gray-300 rounded-full dark:border-gray-800 ltr:mr-2 rtl:ml-2">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
                          <circle cx="12" cy="6" r="4" stroke="currentColor" stroke-width="1.5"></circle>
                          <ellipse opacity="0.5" cx="12" cy="17" rx="7" ry="4" stroke="currentColor" stroke-width="1.5"></ellipse>
                        </svg>
                      </div>
                      <div x-text="userrole.name"></div>
                    </div>
                  </td>
                  <td x-text="userrole.email"></td>
                  <td x-text="userrole.location" class="whitespace-nowrap"></td>
                  <td x-text="userrole.phone" class="whitespace-nowrap"></td>
                  <td>
                    <div class="flex items-center justify-center gap-4">
                      <button type="button" class="btn btn-sm btn-outline-primary" @click="editUserRole(userrole)">Edit</button>
                      <button type="button" class="btn btn-sm btn-outline-danger" @click="deleteUserRole(userrole)">Delete</button>
                    </div>
                  </td>
                </tr>
              </template>
        ...
        <template x-for="userrole in filteredUserRoleList" :key="userrole.id">
          <div class="bg-white dark:bg-[#1c232f] rounded-md overflow-hidden text-center shadow relative">
            <div class="bg-white/40 rounded-t-md bg-[url('/assets/images/notification-bg.png')] bg-center bg-cover p-6 pb-0">
              <template x-if="userrole.path">
                <img class="object-contain w-4/5 mx-auto max-h-40" :src="`/assets/images/${userrole.path}`" />
              </template>
            </div>
            <div class="relative px-6 pb-24 -mt-10">
              <div class="px-2 py-4 bg-white rounded-md shadow-md dark:bg-gray-900">
                <div class="text-xl" x-text="userrole.name"></div>
                <div class="text-white-dark" x-text="userrole.role"></div>
                <div class="flex flex-wrap items-center justify-between gap-3 mt-6">
                  <div class="flex-auto">
                    <div class="text-info" x-text="userrole.posts"></div>
                    <div>Posts</div>
                  </div>
                  <div class="flex-auto">
                    <div class="text-info" x-text="userrole.following"></div>
                    <div>Following</div>
                  </div>
                  <div class="flex-auto">
                    <div class="text-info" x-text="userrole.followers"></div>
                    <div>Followers</div>
                  </div>
                </div>
                <div class="mt-4">
              ...
              <div class="grid grid-cols-1 gap-4 mt-6 ltr:text-left rtl:text-right">
                <div class="flex items-center">
                  ...
                  <div class="truncate text-white-dark" x-text="userrole.email"></div>
                </div>
                <div class="flex items-center">
                  <div class="flex-none ltr:mr-2 rtl:ml-2">Phone :</div>
                  <div class="text-white-dark" x-text="userrole.phone"></div>
                </div>
                <div class="flex items-center">
                  <div class="flex-none ltr:mr-2 rtl:ml-2">Address :</div>
                  <div class="text-white-dark" x-text="userrole.location"></div>
                </div>
              </div>
            </div>
            <div class="absolute bottom-0 flex w-full gap-4 p-6 mt-6 ltr:left-0 rtl:right-0">
              <button type="button" class="w-1/2 btn btn-outline-primary" @click="editUserRole(userrole)">Edit</button>
              <button type="button" class="w-1/2 btn btn-outline-danger" @click="deleteUserRole(userrole)">Delete</button>
            </div>
          </div>
        </template>
      </div>
  ...
  <script>
   document.addEventListener("alpine:init", () => {
			Alpine.data("roles", () => ({
				...
				searchUserRoles() {
					this.filteredUserRoleList = this.userRoleList.filter((d) => d.name.toLowerCase().includes(this.searchUserRole.toLowerCase()));
				},
				editUserRole(userrole) {
					this.params = this.defaultParams;
					if (userrole) {
						this.params = JSON.parse(JSON.stringify(userrole));
					}
				...
				saveUserRole() {
					...
					if (this.params.id) {
						//update userrole
						let userrole = this.userRoleList.find((d) => d.id === this.params.id);
							userrole.name = this.params.name;
							userrole.email = this.params.email;
							userrole.role = this.params.role;
							userrole.phone = this.params.phone;
							userrole.location = this.params.location;
					} else {
						//add userrole
						let maxUserRoleId = this.userRoleList.length ? this.userRoleList.reduce((max, character) => (character.id > max ? character.id : max), this.userRoleList[0].id) : 0;
						let userrole = {
							id: maxUserRoleId + 1,
							path: 'profile-35.png',
						...
						this.userRoleList.splice(0, 0, userrole);
						this.searchUserRoles();
					}
						this.showMessage('User role has been saved successfully.');
						this.addUserRoleModal = false;
				},
				deleteUserRole(userrole) {
					this.userRoleList = this.userRoleList.filter((d) => d.id != userrole.id);
					this.searchUserRoles();
					this.showMessage('User role has been deleted successfully.');
				},
				setDisplayType(type) {
